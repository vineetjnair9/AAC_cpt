...
% Patternsearch
options = optimoptions('particleswarm','Display','off','SwarmSize',100,'MaxIterations',5000,'HybridFcn',@fmincon);
obj_fun = @(x,respondent_num) norm(lottery_obj(x,respondent_num,table))^2;
...
parfor i = 1:length(valid_indices)   
    ...
    nvars = 5;
    [cpt_lottery(i,:),error_norm(i)] = particleswarm(objective,nvars,lb,ub,options); 
end
toc
