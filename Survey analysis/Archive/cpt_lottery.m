...
[cpt_params,resnorm] = estimate_params_lottery(table,'original','lsq');

%% Scatter plots
respondents = 1:1:size(table,1);

figure(1)
scatter(respondents,cpt_params(:,1))
ylabel('\alpha')
xlabel('Respondent')

figure(2)
scatter(respondents,cpt_params(:,2))
ylabel('\beta^+')
xlabel('Respondent')

figure(3)
scatter(respondents,cpt_params(:,3))
ylabel('\beta^-')
xlabel('Respondent')

figure(4)
scatter(respondents,cpt_params(:,4))
ylabel('\lambda')
xlabel('Respondent')
...
%%
function [cpt_lottery,resnorm] = estimate_params_lottery(table,weight_type,method)
    ...
    if strcmp(method,'lsq')
        options = optimoptions('lsqnonlin','FunctionTolerance',1e-14,'OptimalityTolerance',1e-14);
...
%         options.Display = off;
    ...
    for i = 1:num_responses
        ...
        if strcmp(method,'lsq') % Using nonlinear least squares
            [cpt_lottery(i,:),resnorm(i)] = lsqnonlin(fun,x0,lb,ub,options);
        elseif strcmp(method,'gs') % Using grid search
            step = [0.005,0.005,0.005,0.5];
            [cpt_lottery(i,:),resnorm(i)] = gs(fun,lb,ub,step);
        end
...
function error_normalized = calc_error(a,b,c,d,CE_sR,u1,u2,p,R)
    ...
    if (CE_pred(a,b,c,d) == 0 &&  CE_sR(a,b,c,d) == 0)
        error_normalized = 0;
    else 
