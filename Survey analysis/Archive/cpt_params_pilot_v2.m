...
price = -paramhat(7);
ASC_drive = 0; % baseline / paramhat(8)
...
% optimization method = 'lsq' (nonlinear least squares), 'grid' (grid search)
...
%% Plots showing distribution of parameters over respondents
...
function [cpt,resnorm] = estimate_params(table,R_type,weight_type,paramhat,method)
    ...
    sub = @(alpha,beta_plus,beta_minus,lambda,u1,u2,p1,R) subjective(alpha,beta_plus,beta_minus,lambda,u1,u2,p1,R,weight_type);
   
    % Settings for non linear least squares - also try grid search
    lb = [1e-5,1e-5,1e-5,1];
    ub = [1,1,1,100];
    
%     options = optimoptions('lsqnonlin','Display','iter'); % Also record exitflag to confirm optimality of solution?
    options = optimoptions('lsqnonlin','FunctionTolerance',1e-10,'OptimalityTolerance',1e-10); % Also record exitflag to confirm optimality of solution?
    ...
    for i = 1:num_responses
        ...
        elseif table.Reference(i) == 'Exclusive ridesharing'
            ...
            R = ref(u1,u2,p4,CE);
            CE_sR = @(alpha,beta_plus,beta_minus,lambda) -value_func(beta_plus,beta_minus,lambda,CE,R);
            f4 = @(alpha,beta_plus,beta_minus,lambda) sub(alpha,beta_plus,beta_minus,lambda,u1,u2,p2,R) + CE_sR(alpha,beta_plus,beta_minus,lambda);
