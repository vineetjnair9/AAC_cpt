...
tic;
[cpt_lottery,error] = estimate_params_lottery(table,'original','GlobalSearch');
toc;
...
%%
function [cpt_lottery,error] = estimate_params_lottery(table,weight_type,method)
...
%         options.MaxIterations = Inf;
    ...
    for i = 1:num_responses
        ...
        problem = createOptimProblem('fmincon','objective',@(x) sum(fun(x)),'x0',x0,'lb',lb,'ub',ub);
        [cpt_lottery(i,:),error(i)] = run(gs,problem);
