...
obj_fun = @(x,respondent_num) lottery_obj(x,respondent_num,table,reg);
...
name1 =  'cpt_lottery_reg0_MODweight_cdf_norm-maxOutcome.mat';
save(name1,'cpt_lottery')
name2 =  'error_lottery_reg0_MODweight_cdf_norm-maxOutcome.mat';
save(name2,'error')

%% Objective function for each combination of parameter values under consideration
function F = lottery_obj(x,respondent_num,table,reg)
...
%     CE_actual = CE;
    ...
    F = [f1; f2; f3; f4; f5; f6; f7; f8; f9; f10; reg.*[sqrt(norm(x-lb)); sqrt(norm(x-ub))]];
