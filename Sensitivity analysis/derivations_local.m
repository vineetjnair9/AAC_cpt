...
function [dGamma_dTheta_nom,dF_dTheta_nom,opt_soln,opt_cost_actual,mismatch_loss,error,max_dev,rel_sens_soln,rel_sens_cost_actual] = derivations_local(theta,nominal,scenario_vals,devs,plots)
...
% devs: Amount of deviations/perturbations in parameter value (as %)
    ...
    if strcmp(R_val,'Best case') % dynamic
        R = u2;
        U_sR = -exp(-(-log(p))^alpha) * lambda * (R - u1)^beta; 
        A_sR = -lambda*(R - u0)^beta;
        
    elseif strcmp(R_val,'Worst case') % dynamic
        R = u1;
...
%     xlabel('Price ($)'); ylabel('Expected revenue');
    ...
    for i = 1:perturbations
        ...
        mismatch_loss(i) = f_nom_val - double(subs(f,[gamma,theta_val],[opt_price,theta_nom]));
    end
