...
for i = 1:num_iters
    ...
    % After initial condition, update tariff using only feedback until p_star changes
        ...
        p_sR_curr_prev = p_sR1(gamma_prev,alpha_plus_true,alpha_minus_true,beta_plus_true,beta_minus_true,lambda_true);
        p_sR_curr = p_sR1(gamma_curr,alpha_plus_true,alpha_minus_true,beta_plus_true,beta_minus_true,lambda_true);
        
        errors_prev = p_star_vec - p_sR_curr_prev;
        errors_curr = p_star_vec - p_sR_curr;
        ...
        update = alpha.*prev_update + eta.*grad;
...
xlabel('Time (s)','Interpreter','latex');
ylabel('$\|\textbf{p}^s_R - \textbf{p}^*\|$','Interpreter','latex');
yyaxis right
