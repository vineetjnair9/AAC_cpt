...
errors_curr_fixed = zeros(1,num_trips);
errors_curr_time = zeros(1,num_trips);
errors_curr_exp = zeros(1,num_trips);
errors_curr_step = zeros(1,num_trips);

errors_prev_fixed = zeros(1,num_trips);
errors_prev_time = zeros(1,num_trips);
errors_prev_exp = zeros(1,num_trips);
errors_prev_step = zeros(1,num_trips);
...
for i = 1:num_iters
    ...
    if (i == 1) || (p_star(i) ~= p_star(i-1))
        ...
        errors_prev_fixed = errors_curr_fixed;
        errors_prev_time = errors_curr_time;
        errors_prev_exp = errors_curr_exp;
        errors_prev_step = errors_curr_step;
        
        errors_curr_fixed = p_star_vec - p_sR_curr_fixed;
        errors_curr_time = p_star_vec - p_sR_curr_time;
        errors_curr_exp = p_star_vec - p_sR_curr_exp;
        errors_curr_step = p_star_vec - p_sR_curr_step;
             
    % After initial condition, update tariff using only feedback until p_star changes
    else
        p_sR_curr_prev_fixed = p_sR1(gamma_prev_fixed,alpha_plus_true,alpha_minus_true,beta_plus_true,beta_minus_true,lambda_true);
        p_sR_curr_prev_time = p_sR1(gamma_prev_time,alpha_plus_true,alpha_minus_true,beta_plus_true,beta_minus_true,lambda_true);
        p_sR_curr_prev_exp = p_sR1(gamma_prev_exp,alpha_plus_true,alpha_minus_true,beta_plus_true,beta_minus_true,lambda_true);
        p_sR_curr_prev_step = p_sR1(gamma_prev_step,alpha_plus_true,alpha_minus_true,beta_plus_true,beta_minus_true,lambda_true);
        ...
        errors_prev_fixed = p_star_vec - p_sR_curr_prev_fixed;
        errors_prev_time = p_star_vec - p_sR_curr_prev_time;
        errors_prev_exp = p_star_vec - p_sR_curr_prev_exp;
        errors_prev_step = p_star_vec - p_sR_curr_prev_step;
        ...
        grad_fixed = (errors_curr_fixed - errors_prev_fixed)./(gamma_curr_fixed - gamma_prev_fixed);
        grad_time = (errors_curr_time - errors_prev_time)./(gamma_curr_time - gamma_prev_time);
        grad_exp = (errors_curr_exp - errors_prev_exp)./(gamma_curr_exp - gamma_prev_exp);
        grad_step = (errors_curr_step - errors_prev_step)./(gamma_curr_step - gamma_prev_step);
        ...
        % Norm clipping
        grad_fixed = grad_fixed./norm(grad_fixed);
        grad_time = grad_time./norm(grad_time);
        grad_exp = grad_exp./norm(grad_exp);
        grad_step = grad_step./norm(grad_step);
        
        gamma_curr_fixed = gamma_curr_fixed - lr_fixed.*grad_fixed;
        gamma_curr_time = gamma_curr_time - lr_time.*grad_time;
        gamma_curr_exp = gamma_curr_exp - lr_exp.*grad_exp;   
        gamma_curr_step = gamma_curr_step - lr_step.*grad_step;  
