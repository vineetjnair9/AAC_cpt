%% Problem setup
% General reference case with naive error update 
% Old formula
...
for i = 1:num_iters
    ...
    % Desired probability of acceptance at current iteration/sampling time (for all users) 
    ...
    % After initial condition, update tariff using only feedback until p_star changes
        ...
        if normalized
            ...
            gamma_prev = gamma_curr;
            gamma_curr = gamma_currS.*(1 - lr.*grad);
        else
