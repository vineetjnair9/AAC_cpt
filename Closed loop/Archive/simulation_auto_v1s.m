%% Problem setup
% General reference case using naive update (with sub-sampling)
...
for i = 1:num_iters
    ...
    % After initial condition, update tariff using only feedback until p_star changes
        ...
        if normalized
            % Naive update step (normalized)
            error_normalized = errors_currS./max(errors_currS);
            gamma_curr(sampled_users) = gamma_currS - lr.*error_normalized;
        else
            % Naive update step (non-normalized)
            gamma_curr(sampled_users) = gamma_currS - lr.*errors_currS; 
        end
    end
...
% Also plot time-varying gamma for selected passengers/trips
